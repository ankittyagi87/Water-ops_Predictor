import os
import json
import argparse
import logging
import sys
from typing import Dict, List, Literal

import pandas as pd
from pydantic import BaseModel, Field

# ============================================================
# Logging Setup
# ============================================================

def setup_logger(level: str = "INFO", log_dir: str = "logs") -> logging.Logger:
    os.makedirs(log_dir, exist_ok=True)

    logger = logging.getLogger("ingest_pipeline")
    logger.setLevel(getattr(logging, level.upper(), logging.INFO))

    formatter = logging.Formatter(
        "[%(asctime)s] [%(levelname)s] %(name)s :: %(message)s",
        datefmt="%d-%b-%Y %H:%M:%S"
    )

    # Console
    console = logging.StreamHandler(sys.stdout)
    console.setFormatter(formatter)

    # File
    file_handler = logging.FileHandler(
        os.path.join(log_dir, "ingest.log"),
        mode="a",
        encoding="utf-8"
    )
    file_handler.setFormatter(formatter)

    if not logger.handlers:
        logger.addHandler(console)
        logger.addHandler(file_handler)

    logger.propagate = False
    return logger



logger = setup_logger()

# ============================================================
# Pydantic Schema
# ============================================================

class MetricSpec(BaseModel):
    canonical_name: str = Field(..., description="Canonical metric name")

class StreamSpec(BaseModel):
    source_type: Literal["pump", "nozzle", "hydrant"]
    source_id: str
    metrics: Dict[str, MetricSpec]

class TelemetryPydanticConfig(BaseModel):
    streams: Dict[str, StreamSpec]

# ============================================================
# Metric Definitions (numeric only)
# ============================================================

PUMP_METRICS = {
    "engine_rpm": MetricSpec(canonical_name="pump.engine_rpm"),
    "throttle_pct": MetricSpec(canonical_name="pump.throttle_pct"),
    "intake_pressure_psi": MetricSpec(canonical_name="pump.intake_pressure_psi"),
    "discharge_pressure_psi": MetricSpec(canonical_name="pump.discharge_pressure_psi"),
    "tank_level_gal": MetricSpec(canonical_name="pump.tank_level_gal"),
}

NOZZLE_METRICS = {
    "nozzle_pressure_psi": MetricSpec(canonical_name="nozzle.pressure_psi"),
    "flow_gpm": MetricSpec(canonical_name="nozzle.flow_gpm"),
    "nozzle_setting_gpm": MetricSpec(canonical_name="nozzle.setting_gpm"),
}

HYDRANT_METRICS = {
    "static_pressure_psi": MetricSpec(canonical_name="hydrant.static_pressure_psi"),
    "residual_pressure_psi": MetricSpec(canonical_name="hydrant.residual_pressure_psi"),
}

TELEMETRY_CONFIG = TelemetryPydanticConfig(
    streams={
        "pump.jsonl": StreamSpec(
            source_type="pump",
            source_id="pump_id",
            metrics=PUMP_METRICS,
        ),
        "nozzle.jsonl": StreamSpec(
            source_type="nozzle",
            source_id="nozzle_id",
            metrics=NOZZLE_METRICS,
        ),
        "hydrant.jsonl": StreamSpec(
            source_type="hydrant",
            source_id="hydrant_id",
            metrics=HYDRANT_METRICS,
        ),
    }
)

# ============================================================
# Categorical Fields
# ============================================================

CATEGORICAL_FIELDS = {
    "pump": ["valve_state", "priming_active"],
    "nozzle": ["pattern"],
}

# ============================================================
# Labels Loader
# ============================================================

def load_labels(base_path: str) -> pd.DataFrame:
    path = os.path.join(base_path, "labels.csv")
    logger.info(f"Loading labels from {path}")

    if not os.path.exists(path):
        logger.error("labels.csv not found")
        raise FileNotFoundError(path)

    df = pd.read_csv(path)
    df.columns = [c.strip().lower() for c in df.columns]

    required = {"incident_id", "ts_ms", "risk_in_next_120s"}
    if not required.issubset(df.columns):
        logger.error(f"Missing label columns: {required}")
        raise ValueError("Invalid labels schema")

    df["ts_ms"] = df["ts_ms"].astype("int64")
    df["risk_in_next_120s"] = df["risk_in_next_120s"].astype("int8")

    logger.info(f"Loaded {len(df):,} label rows")
    return df[list(required)]

# ============================================================
# Dataset Loader
# ============================================================

class LoadJSONDataset:
    def __init__(self, base_path: str):
        self.base_path = base_path
        self.incidents = [
            d for d in os.listdir(base_path)
            if d.startswith("incident_")
            and os.path.isdir(os.path.join(base_path, d))
        ]
        self.data: Dict[str, Dict[str, List[Dict]]] = {}

        logger.info(f"Found {len(self.incidents)} incidents")

    def _load_jsonl(self, file_path: str) -> List[Dict]:
        records = []
        bad_lines = 0

        if not os.path.exists(file_path):
            logger.warning(f"Missing file: {file_path}")
            return []

        with open(file_path, "r", encoding="utf-8") as f:
            for line_no, line in enumerate(f, 1):
                try:
                    records.append(json.loads(line))
                except json.JSONDecodeError:
                    bad_lines += 1

        if bad_lines:
            logger.warning(
                f"{bad_lines} malformed JSON lines skipped in {file_path}"
            )

        logger.info(f"Loaded {len(records):,} records from {os.path.basename(file_path)}")
        return sorted(records, key=lambda x: x.get("ts_ms", 0))

    def load_all(self):
        for incident in self.incidents:
            logger.info(f"Loading incident {incident}")
            self.data[incident] = {}
            base = os.path.join(self.base_path, incident)

            for file in TELEMETRY_CONFIG.streams:
                path = os.path.join(base, file)
                self.data[incident][file] = self._load_jsonl(path)

    # ========================================================
    # Long Numeric Schema
    # ========================================================

    def to_long_schema(self, incident: str) -> pd.DataFrame:
        rows, dropped = [], 0

        for file, stream in TELEMETRY_CONFIG.streams.items():
            records = self.data[incident].get(file, [])

            for rec in records:
                ts_ms = rec.get("ts_ms")
                source_id = rec.get(stream.source_id)

                if ts_ms is None or source_id is None:
                    dropped += 1
                    continue

                for raw_field, spec in stream.metrics.items():
                    val = rec.get(raw_field)
                    if val is not None:
                        rows.append({
                            "incident_id": incident,
                            "ts_ms": int(ts_ms),
                            "source_type": stream.source_type,
                            "source_id": source_id,
                            "metric": spec.canonical_name,
                            "value": float(val),
                            "event_key": f"{incident}_{source_id}_{ts_ms}_{spec.canonical_name}",
                        })

        logger.info(
            f"{incident}: numeric_rows={len(rows):,}, dropped={dropped:,}"
        )
        return pd.DataFrame(rows)

    # ========================================================
    # Long Categorical Schema
    # ========================================================

    def to_long_schema_categorical(self, incident: str) -> pd.DataFrame:
        rows = []

        for file, stream in TELEMETRY_CONFIG.streams.items():
            fields = CATEGORICAL_FIELDS.get(stream.source_type, [])
            if not fields:
                continue

            records = self.data[incident].get(file, [])

            for rec in records:
                ts_ms = rec.get("ts_ms")
                source_id = rec.get(stream.source_id)

                if ts_ms is None or source_id is None:
                    continue

                for field in fields:
                    val = rec.get(field)
                    if val is not None:
                        rows.append({
                            "incident_id": incident,
                            "ts_ms": int(ts_ms),
                            "source_type": stream.source_type,
                            "source_id": source_id,
                            "metric": f"{stream.source_type}.{field}",
                            "value": str(val),
                            "event_key": f"{incident}_{source_id}_{ts_ms}_{stream.source_type}_{field}",
                        })

        logger.info(f"{incident}: categorical_rows={len(rows):,}")
        return pd.DataFrame(rows)

    # ========================================================
    # Output Writer
    # ========================================================

    def write_outputs(self, out_path: str):
        logger.info(f"Writing outputs to {out_path}")
        os.makedirs(out_path, exist_ok=True)

        numeric_frames, categorical_frames = [], []

        for incident in self.incidents:
            df_num = self.to_long_schema(incident)
            df_cat = self.to_long_schema_categorical(incident)

            if not df_num.empty:
                numeric_frames.append(df_num)
            if not df_cat.empty:
                categorical_frames.append(df_cat)

        telemetry_long = (
            pd.concat(numeric_frames, ignore_index=True)
            if numeric_frames else pd.DataFrame()
        )

        telemetry_cat = (
            pd.concat(categorical_frames, ignore_index=True)
            if categorical_frames else pd.DataFrame()
        )

        labels = load_labels(self.base_path)

        telemetry_long.to_parquet(
            os.path.join(out_path, "telemetry_long.parquet"),
            index=False
        )
        telemetry_cat.to_parquet(
            os.path.join(out_path, "telemetry_categorical.parquet"),
            index=False
        )
        labels.to_parquet(
            os.path.join(out_path, "labels.parquet"),
            index=False
        )

        logger.info("All parquet files written successfully")

# ============================================================
# Main Entrypoint
# ============================================================

def main():
    parser = argparse.ArgumentParser(
        description="Ingest JSONL telemetry into curated Parquet datasets"
    )
    parser.add_argument("--input", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--log-level", default="INFO")
    args = parser.parse_args()

    global logger
    logger = setup_logger(args.log_level)

    logger.info("Starting telemetry ingestion pipeline")

    dataset = LoadJSONDataset(base_path=args.input)
    dataset.load_all()
    dataset.write_outputs(out_path=args.output)

    logger.info("Telemetry ingestion completed successfully")

if __name__ == "__main__":
    main()
